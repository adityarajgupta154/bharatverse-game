...
- Ambient auto-speak stagger must EXCEED the bubble's open duration, or every page entry shows an N-bubble chorus for the first several seconds (stagger 1.7s + open 3.8s ⇒ 3 open at once). Chain them: first-speak delay step ≥ openMs + gap; steady-state periods can then drift freely.
- A static reference mock that shows many bubbles simultaneously is an "all dialogue at once" ILLUSTRATION, not a live-behavior spec — don't copy it literally; pick the live pacing that keeps the painting readable (hover reveals any line on demand anyway).
